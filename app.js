...
// ─── WITHDRAW BANK SELECT (no more "new account" option — users must save via Settings) ───
// Toggle removed since new account entry was removed from withdraw modal
...
(async () => {
  ...
  function applyRuntimeConfigToUI() {
    ...
    // ═══ Update referral percentage display ═══
    const referralPct = runtimePlatformConfig.referralPercentage || DEFAULT_PLATFORM_CONFIG.referralPercentage || 25;
    const pctEls = ['referralPctDisplay', 'refModalPct'];
    pctEls.forEach(id => {
      const el = document.getElementById(id);
  ...
  async function processAutoDailyReturns() {
    ...
    // ═══ FIX: Daily returns settle at 12:02 AM only ═══
    const now = new Date();
    const currentHour = now.getHours();
    const currentMinute = now.getMinutes();

    // Don't process before 12:02 AM (00:02)
    if (currentHour === 0 && currentMinute < 3) return;

    const today = getTodayString();

    // Today's settlement cutoff: 12:02 AM today
    // Plans purchased AFTER this cutoff won't receive returns until tomorrow
    const todayCutoff = new Date(now.getFullYear(), now.getMonth(), now.getDate(), 0, 3, 0);
    ...
    try {
      ...
      for (const invDoc of investSnap.docs) {
        ...
        // ═══ FIX: Skip plans purchased after today's 12:02 AM cutoff ═══
        // These plans will get their first return at tomorrow's 12:02 AM
        const investCreatedAt = inv.createdAt || inv.startDate;
        if (investCreatedAt) {
          const createdDate = investCreatedAt.toDate ? investCreatedAt.toDate() : new Date(investCreatedAt);
  ...
  window.submitWithdrawal = async () => {
    ...
    try {
      ...
      if (!account) {
        isWithdrawProcessing = false;
        return showToast("Please select a saved bank account. Add one in Settings → Saved Bank Accounts.", "error");
      }
  ...
  function updateBankSelect() {
    ...
    // No more "+ Enter new account" — users must save accounts in Settings first
  }
  ...
  function renderTransactions(transactions, containerId) {
    ...
    container.innerHTML = transactions.map(t => {
      ...
      const iconClass= t.type === "deposit" ? "fa-circle-arrow-down" : t.type === "withdraw" ? "fa-arrow-up-from-bracket" : t.type === "daily_return" ? "fa-coins" : "fa-seedling";
      const label    = t.type === "deposit" ? "Deposit" : t.type === "withdraw" ? "Withdrawal" : t.type === "daily_return" ? "Daily Return" : "Investment — " + (t.plan || "");
      const displayStatus = (t.type === "withdraw" && t.status === "approved") ? "Successful" : t.status;
      const statusClass = (t.type === "withdraw" && t.status === "approved") ? "successful" : t.status;
      return `
        <div class="txn-item">
          ...
          <div class="txn-info">
            ...
            <span class="txn-status status-${statusClass}">${displayStatus}</span>
          </div>
          <div>
  ...
  // ═══════════════════════════════════════════════════════════════
  //  REFERRAL DASHBOARD — Show referred friends, deposits, earnings
  // ═══════════════════════════════════════════════════════════════

  async function loadReferralDashboard() {
    if (!currentUser || !currentUserData?.referralCode) return;
    try {
      // Find all users referred by current user
      const refSnap = await getDocs(
        query(collection(db, "users"), where("referredBy", "==", currentUserData.referralCode))
      );
      ...
      for (const friendDoc of refSnap.docs) {
        const fData = friendDoc.data();
        // Get all approved deposits for this friend
        ...
        let friendTotalDeposits = 0;
        depSnap.docs.forEach(d => {
          friendTotalDeposits += Number(d.data().amount || 0);
        });
        const friendEarnings = Math.floor(friendTotalDeposits * referralPct / 100);
        ...
        friends.push({
          ...
          phone: fData.phone || "",
          totalDeposits: friendTotalDeposits,
          earnings: friendEarnings
        });
      }

      // Update home card stats
      const friendsEl = document.getElementById("refTotalFriends");
      const depositsEl = document.getElementById("refTotalDeposits");
      const earningsEl = document.getElementById("refTotalEarnings");
      if (friendsEl) friendsEl.textContent = friends.length;
      if (depositsEl) depositsEl.textContent = fmt(totalDeposits);
      if (earningsEl) earningsEl.textContent = fmt(totalEarnings);

      // Update modal stats
      const mFriends = document.getElementById("refModalFriends");
      const mEarnings = document.getElementById("refModalEarnings");
      if (mFriends) mFriends.textContent = friends.length;
      if (mEarnings) mEarnings.textContent = fmt(totalEarnings);

      // Render friends list in modal
      const listEl = document.getElementById("referralFriendsList");
      if (listEl) {
        if (!friends.length) {
          listEl.innerHTML = '<div class="empty-state sm"><i class="fas fa-user-plus"></i><p>No referrals yet. Share your code!</p></div>';
        } else {
          listEl.innerHTML = friends.map(f => {
            const initials = (f.name || "U").split(" ").map(w => w[0]).join("").toUpperCase().slice(0,2);
            const maskedPhone = f.phone ? f.phone.slice(0,4) + "****" + f.phone.slice(-3) : "";
            return \`
              <div class="ref-friend-item">
                <div class="ref-friend-avatar">\${initials}</div>
                <div class="ref-friend-info">
                  <div class="ref-friend-name">\${f.name}</div>
                  <div class="ref-friend-deposits">\${maskedPhone} · Deposited: \${fmt(f.totalDeposits)}</div>
                </div>
                <div class="ref-friend-earned">
                  <span class="ref-friend-earned-val">\${fmt(f.earnings)}</span>
                  <span class="ref-friend-earned-label">Earned</span>
                </div>
              </div>
            \`;
          }).join("");
        ...
        return `
          <div class="proof-feed-card" onclick="viewProofImage('${d.id}')">
            <div class="proof-feed-top">
              ...
              <div class="proof-feed-verified"><i class="fas fa-circle-check"></i> Verified</div>
            </div>
  ...
  // ═══════════════════════════════════════════════════════════════
  //  FLOATING CUSTOMER CARE BUTTON — Draggable + Click to open link
  // ═══════════════════════════════════════════════════════════════
  (function setupFloatingCustomerCare() {
    const btn = document.getElementById('floatingCustomerCare');
    if (!btn) return;
    ...
    function snapToEdge(x, y) {
      ...
      const finalX = (x + w / 2) < midX ? 14 : window.innerWidth - w - 14;
      return { x: finalX, y: y };
    }

    function applyPosition(x, y, animate) {
      btn.style.transition = animate ? 'left 0.3s ease, top 0.3s ease' : 'none';
    ...
    btn.addEventListener('touchmove', (e) => {
      ...
      if (wasDragged) {
        ...
        applyPosition(pos.x, pos.y, false);
      }
    }, { passive: false });
    ...
    btn.addEventListener('touchend', (e) => {
      ...
      if (wasDragged) {
        ...
        const constrained = constrainPosition(snapped.x, snapped.y);
        applyPosition(constrained.x, constrained.y, true);
        btn.classList.remove('dragging');
      } else {
        openCustomerCareFromFloat();
      }
    }, { passive: true });
    ...
    function openCustomerCareFromFloat() {
      const link = runtimeLinksConfig.customerCareLink;
      if (link) {
        window.open(link, '_blank');
      } else {
        showToast('Customer care link not available. Please contact support.', 'warning');
      }
    }

    // Show/hide based on customer care link
    function updateFloatingBtnVisibility() {
      if (runtimeLinksConfig.customerCareLink) {
    ...
    setTimeout(updateFloatingBtnVisibility, 3000);
    setTimeout(updateFloatingBtnVisibility, 6000);

    // Hook into config load
    const origApplyConfigForFloat = applyRuntimeConfigToUI;
    applyRuntimeConfigToUI = function() {
      origApplyConfigForFloat();
      updateFloatingBtnVisibility();
    };
  })();

  // ═══════════════════════════════════════════════════════════════
  //  UNIVERSAL BUTTON GUARD — Prevent double-clicks on ALL buttons
  // ═══════════════════════════════════════════════════════════════
  (function setupUniversalButtonGuard() {
    document.addEventListener('click', function(e) {
      const btn = e.target.closest('button, .btn-primary, .btn-secondary, .wallet-btn, .plan-invest-btn, .qa-btn');
      if (!btn || btn.classList.contains('btn-processing') || btn.classList.contains('btn-loading')) return;
      if (btn.classList.contains('modal-close') || btn.classList.contains('eye-btn') || btn.classList.contains('copy-btn') || btn.classList.contains('tab-btn') || btn.classList.contains('txn-filter') || btn.classList.contains('nav-item') || btn.classList.contains('bt-tab') || btn.classList.contains('deposit-method-tab') || btn.classList.contains('icon-btn') || btn.classList.contains('see-all-btn') || btn.classList.contains('toggle-switch')) return;
      // Only guard buttons that trigger async actions
      const onclick = btn.getAttribute('onclick') || '';
      if (!onclick && !btn.dataset.asyncAction) return;
      if (onclick.includes('switch') || onclick.includes('filter') || onclick.includes('toggle') || onclick.includes('set') && onclick.includes('Amount')) return;
      
      btn.classList.add('btn-processing');
      const origHtml = btn.innerHTML;
      // Add spinner
      btn.innerHTML = '<span class="btn-spin"></span> Processing...';
      
      // Auto-restore after max 8 seconds
      setTimeout(() => {
  ...
  console.log(`SliceInvest v4.2 initialized (${backendMode} mode)`);
})();
