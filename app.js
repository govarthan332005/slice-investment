  ...
  async function syncActiveInvestmentsWithPlans() {
    ...
    try {
      ...
      for (var i = 0; i < investSnap.docs.length; i++) {
        ...
        if (matchedPlan && matchedPlan.dailyReturnFixed !== inv.dailyReturnFixed) {
          console.log("[SliceInvest] Syncing investment", invDoc.id, "daily return:", inv.dailyReturnFixed, "->", matchedPlan.dailyReturnFixed);
          await updateDoc(doc(db, "investments", invDoc.id), {
            dailyReturnFixed: matchedPlan.dailyReturnFixed
          });
        }
      }
    } catch(err) {
