// ============================================================
//  SLICE INVEST — Enhanced App v3.0
//  Updated: New Plans, Auto Dispersal, Withdraw Restrictions,
//           Referral on Deposit, Per-Plan Purchase Limit,
//           UPI/Bank Withdraw Toggles
// ============================================================
...
(async () => {
  ...
  const MAX_PURCHASES_PER_PLAN = 2;
  ...
  // ─── USER SIGNUP (UPDATED: No referral bonus on signup) ───────
  window.userSignup = async () => {
    ...
    try {
      ...
      // ═══ UPDATED: No referral bonus on signup. Bonus given when referred user deposits ₹500+ ═══
      const newUserRef = await addDoc(collection(db, "users"), {
        ...
        referralBonusPaid: false, // Track if referral bonus was already given
        dailyRewardStreak: 0, lastRewardDate: null, rewardHistory: [],
        darkMode: false, notificationsEnabled: true,
        createdAt: serverTimestamp()
      });

      // ═══ NO referral bonus here — will be triggered on first deposit ≥ ₹500 ═══
  ...
  function initUserDashboard() {
    ...
    renderDailyCalendar();
    processAutoDailyReturns(); // ═══ NEW: Auto-disperse daily returns ═══
  ...
  // ─── Invest (UPDATED: Max 2 per plan + term tracking) ─────────
  window.openInvestModal = (planId) => {
    ...
    document.getElementById("selectedPlanPreview").innerHTML = `
      ...
      <div class="spp-detail"><i class="fas fa-repeat"></i> Max ${MAX_PURCHASES_PER_PLAN} Purchases</div>
    `;
  ...
  window.confirmInvest = async () => {
    ...
    try {
      // ═══ CHECK: Max 2 active purchases per plan ═══
      ...
      if (investSnap.size >= MAX_PURCHASES_PER_PLAN) {
        isInvestProcessing = false;
        return showToast(`You can only have ${MAX_PURCHASES_PER_PLAN} active purchases of ${selectedPlan.name}. Wait for current plans to complete.`, "error");
      }

      // Re-read latest balance
  ...
  //  Checks all active investments and credits daily returns
  // ═══════════════════════════════════════════════════════════════
  ...
  async function processAutoDailyReturns() {
    ...
    try {
      ...
      if (totalDailyCredit > 0) {
        // Credit to user's balance AND withdrawable balance
        const freshSnap = await getDoc(doc(db, "users", currentUser));
        if (freshSnap.exists()) {
          const freshData = freshSnap.data();
      ...
      // Update active plans count if any completed
  ...
  window.submitWithdrawal = async () => {
    ...
    try {
      ...
      // ═══ RESTRICTION: Can only withdraw from plan earnings (withdrawableBalance) ═══
      if (withdrawableBalance < amount) {
        isWithdrawProcessing = false;
        return showToast(`Insufficient withdrawable funds. You can only withdraw earnings from investment plans. Available: ${fmt(withdrawableBalance)}`, "error");
      }
      ...
      if (bankSel && bankSel !== "new" && bankSel !== "") {
        ...
        if (bank) {
          if (bank.type === "upi") {
            // ═══ CHECK: UPI withdrawals enabled? ═══
            if (!runtimeWithdrawConfig.upiWithdrawEnabled) {
              isWithdrawProcessing = false;
              return showToast("UPI withdrawals are currently disabled by admin. Please use a bank account.", "error");
            }
            account = `UPI: ${bank.upiId}`;
            bankDetails = { type: "upi", upiId: bank.upiId, displayName: bank.displayName || "" };
            withdrawMethod = "upi";
          } else {
            // ═══ CHECK: Bank withdrawals enabled? ═══
            if (!runtimeWithdrawConfig.bankWithdrawEnabled) {
              isWithdrawProcessing = false;
              return showToast("Bank account withdrawals are currently disabled by admin. Please use UPI.", "error");
            }
  ...
  // ─── Load My Investments (UPDATED: Show term progress) ────────
  ...
  let notificationDocs = []; // Store notification doc references
  ...
  async function loadNotifications() {
    try {
      ...
      notificationDocs = docs; // Store for click handling
      ...
      document.getElementById("notificationsList").innerHTML = docs.map((d, index) => {
        ...
        return `
          <div class="notif-item ${readClass}" data-notif-id="${d.id}" data-notif-index="${index}" onclick="markNotificationRead('${d.id}', this)">
  ...
  window.markNotificationRead = async (notifId, element) => {
    ...
    try {
      ...
      // After animation, apply final read state
      setTimeout(() => {
  ...
  window.markAllNotificationsRead = async () => {
    try {
      ...
      // Animate all unread items
      unreadItems.forEach(item => {
        item.classList.remove("notif-unread");
        item.classList.add("notif-marking-read");
      });
      ...
      for (const nd of notificationDocs) {
        ...
        if (!data.read) {
          updatePromises.push(
            updateDoc(doc(db, "notifications", nd.id), { read: true })
          );
        }
      }
      await Promise.all(updatePromises);

      // Apply final read state after animation
      setTimeout(() => {
        unreadItems.forEach(item => {
          item.classList.remove("notif-marking-read");
  ...
  function renderDailyCalendar() {
    ...
    if (completedDays > MAX_REWARD_STREAK) completedDays = 0;
  ...
  // ═══════════════════════════════════════════════════════════════
  //  PULL-TO-REFRESH (Zomato-style touch gesture)
  // ═══════════════════════════════════════════════════════════════
  (function setupPullToRefresh() {
    ...
    const PTR_THRESHOLD = 70;      // px to pull before triggering refresh
    const PTR_MAX_PULL = 110;      // max pull distance
    const PTR_RESISTANCE = 2.5;    // pull resistance factor
    ...
    function getActiveSection() {
      const sections = ['homeSection', 'investSection', 'walletSection'];
    ...
    async function performRefresh() {
      ...
      try {
        ...
        if (activeSection === 'homeSection') {
          ...
          renderDailyCalendar();
          processAutoDailyReturns();
        } else if (activeSection === 'walletSection') {
          await loadTransactions();
          await loadBankAccounts();
        } else if (activeSection === 'investSection') {
          await loadMyInvestments();
          renderPlans();
        }
    ...
    mainEl.addEventListener('touchstart', (e) => {
      ...
      // Only allow PTR on refreshable tabs
      const activeSection = getActiveSection();
    ...
    mainEl.addEventListener('touchmove', (e) => {
      ...
      const rawDistance = touchCurrentY - touchStartY;
      ...
      // Apply resistance curve (feels like Zomato/Instagram)
      pullDistance = Math.min(PTR_MAX_PULL, rawDistance / PTR_RESISTANCE);

      // Prevent default scroll when pulling
      if (pullDistance > 5 && isAtTop()) {
        e.preventDefault();
      }
      ...
      mainEl.classList.add('ptr-active', 'ptr-pulling');

      // Rotate spinner based on pull distance
      const rotation = (pullDistance / PTR_MAX_PULL) * 360;
      if (ptrSpinner) ptrSpinner.style.transform = `rotate(${rotation}deg)`;

      // Update text and arrow based on threshold
      if (pullDistance >= PTR_THRESHOLD) {
        ptrEl.classList.add('release-ready');
        if (ptrText) ptrText.textContent = 'Release to refresh';
      } else {
        ptrEl.classList.remove('release-ready');
    ...
    function resetPull() {
      isPulling = false;
      pullDistance = 0;
      ptrEl.classList.remove('pulling', 'release-ready');
      ptrEl.style.height = '0px';
      mainEl.classList.remove('ptr-active', 'ptr-pulling');
    ...
    async function triggerRefresh() {
      ...
      mainEl.classList.remove('ptr-pulling');
      if (ptrSpinner) ptrSpinner.style.transform = '';
      if (ptrText) ptrText.textContent = 'Refreshing...';
      if (ptrArrow) ptrArrow.style.display = 'none';

      // Add ripple effect
      const ripple = document.createElement('div');
      ripple.className = 'ptr-ripple';
      ptrEl.appendChild(ripple);
      setTimeout(() => ripple.remove(), 600);

      // Perform refresh
      await performRefresh();

      // Animate sections after refresh
      const activeSection = getActiveSection();
      if (activeSection) {
        const sections = document.getElementById(activeSection)?.querySelectorAll('section');
        if (sections) {
          sections.forEach(s => {
      ...
      // Reset after small delay so user sees the result
      setTimeout(() => {
        isRefreshing = false;
        ptrEl.classList.remove('refreshing');
        ptrEl.style.height = '0px';
        mainEl.classList.remove('ptr-active');
        if (ptrText) ptrText.textContent = 'Pull down to refresh';
        if (ptrArrow) ptrArrow.style.display = '';
      }, 500);
    }
  })();

  // ─── Withdraw Modal — Check if disabled (with UPI/Bank specifics) ──
  ...
  window.openModal = (id) => {
    ...
    if (id === 'withdrawModal') {
      if (!runtimeWithdrawConfig.apiWithdrawEnabled) {
        ...
        // Show info about withdrawable balance
        const withdrawableInfo = document.getElementById('withdrawableInfo');
        if (withdrawableInfo) {
          const wb = currentUserData?.withdrawableBalance || 0;
  ...
  console.log(`SliceInvest v3.0 initialized (${backendMode} mode)`);
})();
