...
(async function() {
  ...
  var runtimeDepositConfig = { enableUpi: true, enableQr: false, enableBank: false, qrCodeImage: '', bankAccountName: '', bankAccountNumber: '', bankIfsc: '', bankName: '' };
  ...
  function applyDepositMethodsUI() {
    ...
    if (cfg.bankAccountNumber) {
      ...
      if (bankContainer) {
        bankContainer.innerHTML = '<div class="deposit-bank-row"><span class="dbr-label">Bank Name</span><span class="dbr-value">' + (cfg.bankName || '—') + '</span><button class="deposit-bank-copy" onclick="copyToClipboard(\'' + (cfg.bankName || '').replace(/'/g, "\\'") + '\')"><i class="fas fa-copy"></i></button></div><div class="deposit-bank-row"><span class="dbr-label">Account Name</span><span class="dbr-value">' + (cfg.bankAccountName || '—') + '</span><button class="deposit-bank-copy" onclick="copyToClipboard(\'' + (cfg.bankAccountName || '').replace(/'/g, "\\'") + '\')"><i class="fas fa-copy"></i></button></div><div class="deposit-bank-row"><span class="dbr-label">Account No.</span><span class="dbr-value">' + (cfg.bankAccountNumber || '—') + '</span><button class="deposit-bank-copy" onclick="copyToClipboard(\'' + (cfg.bankAccountNumber || '').replace(/'/g, "\\'") + '\')"><i class="fas fa-copy"></i></button></div><div class="deposit-bank-row"><span class="dbr-label">IFSC Code</span><span class="dbr-value">' + (cfg.bankIfsc || '—') + '</span><button class="deposit-bank-copy" onclick="copyToClipboard(\'' + (cfg.bankIfsc || '').replace(/'/g, "\\'") + '\')"><i class="fas fa-copy"></i></button></div>';
      }
    }
  }
  ...
  function saveUserSession(userId) {
    try {
      localStorage.setItem(SESSION_KEY, JSON.stringify({ userId: userId, timestamp: Date.now() }));
    ...
    grid.innerHTML = PLANS.map(function(p) {
      return '<div class="plan-card ' + p.badge + '">' +
        ...
        '<div class="plan-features">' +
          (p.features || []).map(function(f) { return '<div class="plan-feature"><i class="fas fa-check-circle"></i>' + f + '</div>'; }).join("") +
        '</div>' +
  ...
  async function processAutoDailyReturns() {
    ...
    var now = new Date();
    if (now.getHours() === 0 && now.getMinutes() < 3) return;
    ...
    try {
      ...
      if (totalDailyCredit > 0) {
        ...
        if (freshSnap.exists()) {
          ...
          await updateDoc(doc(db, "users", currentUser), {
            balance: (freshData.balance || 0) + totalDailyCredit,
            withdrawableBalance: (freshData.withdrawableBalance || 0) + totalDailyCredit,
            totalReturns: (freshData.totalReturns || 0) + totalDailyCredit,
            activePlans: Math.max(0, (freshData.activePlans || 0) - plansCompleted)
          });
          await addDoc(collection(db, "transactions"), { userId: currentUser, type: "daily_return", amount: totalDailyCredit, plan: "Daily Investment Returns", status: "approved", createdAt: serverTimestamp() });
          await addDoc(collection(db, "notifications"), { userId: currentUser, message: "💰 Daily returns of " + fmt(totalDailyCredit) + " credited!", type: "success", read: false, createdAt: serverTimestamp() });
      ...
      if (plansCompleted > 0 && totalDailyCredit === 0) {
        var freshSnap2 = await getDoc(doc(db, "users", currentUser));
        if (freshSnap2.exists()) {
          await updateDoc(doc(db, "users", currentUser), {
            activePlans: Math.max(0, (freshSnap2.data().activePlans || 0) - plansCompleted)
          });
          loadUserData();
        }
      }
    } catch(err) { console.error("Auto daily returns error:", err); }
  }
  ...
  window.submitWithdrawal = async function() {
    ...
    try {
      ...
      var newWithdrawable = withdrawableBalance - amount;
      ...
      var withdrawRequest = {
        userId: currentUser, userName: currentUserData.name, userPhone: currentUserData.phone,
        type: "withdraw", amount: amount, account: account, withdrawMethod: withdrawMethod,
      ...
      await addDoc(collection(db, "transactions"), { userId: currentUser, type: "withdraw", amount: amount, account: account, status: "pending", createdAt: serverTimestamp() });
  ...
  function renderTransactions(transactions, containerId) {
    ...
    container.innerHTML = transactions.map(function(t) {
      var isCredit = t.type === "deposit" || t.type === "daily_return";
      var typeIcon = t.type === "deposit" ? "deposit" : t.type === "withdraw" ? "withdraw" : t.type === "daily_return" ? "deposit" : "invest";
      var iconClass= t.type === "deposit" ? "fa-circle-arrow-down" : t.type === "withdraw" ? "fa-arrow-up-from-bracket" : t.type === "daily_return" ? "fa-coins" : "fa-seedling";
      var label    = t.type === "deposit" ? "Deposit" : t.type === "withdraw" ? "Withdrawal" : t.type === "daily_return" ? "Daily Return" : "Investment — " + (t.plan || "");
      ...
      return '<div class="txn-item"><div class="txn-icon ' + typeIcon + '"><i class="fas ' + iconClass + '"></i></div><div class="txn-info"><div class="txn-type">' + label + '</div><div class="txn-date">' + fmtDateTime(t.createdAt) + '</div><span class="txn-status status-' + statusClass + '">' + displayStatus + '</span></div><div><div class="txn-amount ' + (isCredit ? 'credit':'debit') + '">' + (isCredit?'+':'-') + fmt(t.amount) + '</div></div></div>';
    }).join("");
  }
  ...
  window.filterTransactions = function(type, btn) {
    ...
    btn.classList.add("active");
    var filtered = type === "all" ? allTransactions : allTransactions.filter(function(t) { return t.type === type; });
  ...
  function loadDailyRewardStatus() {
    ...
    var banner = document.getElementById("dailyRewardBanner");
    var subtextEl = document.getElementById("dailyRewardSubtext");
    if (lastReward) {
      var lastDate;
      try { lastDate = lastReward.toDate ? lastReward.toDate() : new Date(lastReward); } catch(e) { lastDate = new Date(0); }
  ...
  window.claimDailyReward = async function() {
    ...
    if (lastReward) {
      var lastDate;
      try { lastDate = lastReward.toDate ? lastReward.toDate() : new Date(lastReward); } catch(e) { lastDate = new Date(0); }
      if (lastDate.toDateString() === today) return showToast("Already claimed today! 🕐", "warning");
      var yesterday = new Date(); yesterday.setDate(yesterday.getDate() - 1);
      if (lastDate.toDateString() !== yesterday.toDateString()) { if (currentUserData) currentUserData.dailyRewardStreak = 0; }
    }
    ...
    var baseReward = Number(runtimePlatformConfig.baseReward || DEFAULT_PLATFORM_CONFIG.baseReward);
    var streakBonus = Number(runtimePlatformConfig.streakBonus || DEFAULT_PLATFORM_CONFIG.streakBonus);
    var maxReward = Number(runtimePlatformConfig.maxReward || DEFAULT_PLATFORM_CONFIG.maxReward);
    var bonus = Math.min(baseReward + streak * streakBonus, maxReward);
    try {
      ...
      if (currentUserData) { currentUserData.balance = newBalance; currentUserData.dailyRewardStreak = streak; currentUserData.rewardHistory = recentHistory; currentUserData.lastRewardDate = { toDate: function() { return new Date(); } }; }
      ...
      showToast("🎁 ₹" + bonus + " bonus claimed! Day " + streak + "!", "success");
    } catch(err) { showToast("Failed: " + err.message, "error"); }
  };
  ...
  async function loadWithdrawProofs() {
    ...
    try {
      ...
      container.innerHTML = docs.map(function(d) {
        ...
        var timeAgo = getTimeAgo(data.verifiedAt || data.createdAt);
